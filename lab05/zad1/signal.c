...
static void set_handlers_using_signal();
static inline void check_signal_handler(void (*handler)(int));
...
static void set_handlers_using_signal()
{
	...
	handler = signal(SIGINT, signal_interrupt_handler);
	check_signal_handler(handler);
	
	handler = signal(SIGTSTP, signal_tstp_handler);
	check_signal_handler(handler);
}
