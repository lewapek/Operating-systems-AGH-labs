...
int main(int argc, char const *argv[])
{
	...
	sigset_t mask;
	sigemptyset(&mask);
	sigaddset(&mask, SIGRTMIN + SIGUSR1);
	sigaddset(&mask, SIGRTMIN + SIGUSR2);
	...
	int i, check;
	for(i = 0; i < received_signals; ++i)
	{
		check = kill(ppid, SIGRTMIN + SIGUSR1);
		check_send_by_kill(check);
	}

	check = kill(ppid, SIGRTMIN + SIGUSR2);
	check_send_by_kill(check);
...
static void set_sigaction()
{
	...
	check = sigaction(SIGRTMIN + SIGUSR1, &sigusr1, NULL);
	sigaction_check(check);
	check = sigaction(SIGRTMIN + SIGUSR2, &sigusr2, NULL);
	sigaction_check(check);
}
